...
<div class="max-w-4xl">
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-body mb-2">
            ...
            <span class="text-heading">Edit Anggota</span>
        </div>
        <h1 class="text-2xl font-bold text-heading">Edit Anggota Team</h1>
        <p class="text-sm text-body mt-1">Edit informasi anggota team</p>
    </div>

    <div class="bg-neutral-primary-soft border border-default rounded-base p-6">
        <form action="<?= url('admin/employees/' . $employee->id) ?>" method="POST" enctype="multipart/form-data" x-data="employeeForm()">
            <?= csrf_field() ?>
            <?= method_field('PUT') ?>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    >
                        <?php foreach ($teams as $team): ?>
                            <option value="<?= $team->id ?>" <?= ($team->id === old('team_id', $employee->team_id ?? '')) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <?php if (!empty($employee->photo)): ?>
                        ...
                        ?>
                        <div class="mb-3">
                            <p class="text-xs text-body mb-2">Foto saat ini:</p>
                            <img src="<?= asset($photoPath) ?>" alt="Current photo" class="w-32 h-32 object-cover rounded-full border border-default">
                        </div>
                    <?php endif; ?>
                    
                    <!-- New Photo Preview -->
                    <div x-show="photoPreview" class="mb-3 relative inline-block">
                        <p class="text-xs text-body mb-2">Preview foto baru:</p>
                        <img :src="photoPreview" class="w-32 h-32 object-cover rounded-full border border-default">
                        <button 
                            type="button" 
                            @click="removePhoto()"
                            class="absolute top-6 right-0 p-1.5 bg-danger text-white rounded-full hover:bg-danger-strong shadow"
                        >
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                    <p class="text-xs text-body mt-2">Upload foto baru untuk mengganti. Format: JPG, PNG, WebP. Maksimal 5MB</p>
