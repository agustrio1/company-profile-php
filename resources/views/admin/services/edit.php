...
<div class="max-w-4xl">
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-body mb-2">
            ...
            <span class="text-heading">Edit Service</span>
        </div>
        <h1 class="text-2xl font-bold text-heading">Edit Service</h1>
        <p class="text-sm text-body mt-1">Edit layanan yang sudah ada</p>
    </div>

    <div class="bg-neutral-primary-soft border border-default rounded-base p-6">
        <form action="<?= url('admin/services/' . $service->id) ?>" method="POST" enctype="multipart/form-data" x-data="serviceForm()">
            <?= csrf_field() ?>
            <?= method_field('PUT') ?>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <?php if (!empty($service->icon)): ?>
                        ...
                        ?>
                        <div class="mb-3">
                            <p class="text-xs text-body mb-2">Icon saat ini:</p>
                            <img src="<?= asset($iconPath) ?>" alt="Current icon" class="w-24 h-24 object-contain border border-default rounded p-2">
                        </div>
                    <?php endif; ?>
                    
                    <!-- New Icon Preview -->
                    <div x-show="iconPreview" class="mb-3 relative inline-block">
                        <p class="text-xs text-body mb-2">Preview icon baru:</p>
                    ...
                    >
                    <p class="text-xs text-body mt-2">Upload icon baru untuk mengganti. Format: PNG, SVG. Maksimal 2MB</p>
                ...
                <div>
                    ...
                    <?php if (!empty($service->image)): ?>
                        ...
                        <div class="mb-3">
                            ...
                            <img src="<?= asset($imagePath) ?>" alt="Current image" class="w-48 h-32 object-cover rounded-base border border-default">
                        </div>
                    <?php endif; ?>
                ...
                <div class="border-t border-default pt-5">
                    ...
                    <div class="mb-4">
                        ...
                        <input 
                            ...
                            id="seo_title" 
                            name="seo[title]" 
                            value="<?= old('seo.title', $service->seo->title ?? '') ?>"
                            class="bg-neutral-primary border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
                    ...
                    <div class="mb-4">
                        ...
                        ><?= old('seo.description', $service->seo->description ?? '') ?></textarea>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="seo_keywords" 
                            name="seo[keywords]" 
                            value="<?= old('seo.keywords', $service->seo->keywords ?? '') ?>"
                            class="bg-neutral-primary border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
...
function serviceForm() {
    return {
        ...
        slug: '<?= old('slug', addslashes($service->slug ?? '')) ?>',
        autoSlug: false,
        iconPreview: null,
        imagePreview: null,
