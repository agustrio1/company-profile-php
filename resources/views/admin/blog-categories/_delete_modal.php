...
>
    ...
    >
        <div class="flex items-start mb-4">
            <div class="w-12 h-12 <?= $hasBlog ? 'bg-warning-soft' : 'bg-danger-soft' ?> rounded-full flex items-center justify-center mr-4 flex-shrink-0">
                <svg class="w-6 h-6 <?= $hasBlog ? 'text-fg-warning' : 'text-fg-danger' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
            </div>
            <div class="flex-1">
                <h3 class="text-lg font-semibold text-heading mb-1">
                    <?= $hasBlog ? 'Kategori Tidak Dapat Dihapus' : 'Hapus Kategori' ?>
                </h3>
                
                <?php if ($hasBlog): ?>
                    <p class="text-sm text-body">
                        Kategori <strong><?= htmlspecialchars($category->name ?? '') ?></strong> masih memiliki <strong><?= $blogCount ?> blog</strong>.
                    </p>
                    <p class="text-sm text-body mt-2">
                        Hapus atau pindahkan semua blog dari kategori ini terlebih dahulu.
                    </p>
                <?php else: ?>
                    <p class="text-sm text-body">
                        Apakah Anda yakin ingin menghapus kategori <strong><?= htmlspecialchars($category->name ?? '') ?></strong>?
                    </p>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button 
                ...
                @click="deleting = true"
                :disabled="deleting"
                hx-post="<?= url('admin/blog-categories/' . ($category->id ?? '')) ?>"
