...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-heading">Manajemen Users</h1>
            <p class="text-sm text-body mt-1">Kelola pengguna dan akses sistem</p>
        </div>
        <a href="<?= url('admin/users/create') ?>" class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-white bg-brand hover:bg-brand-strong focus:outline-none focus:ring-4 focus:ring-brand-medium rounded-base transition-colors">
    ...
    <div id="users-table" 
         hx-get="<?= url('admin/users/table') ?>"
         hx-trigger="load, userDeleted from:body">
        <!-- Loading indicator -->
...
// HTMX Event - After successful delete
document.body.addEventListener('htmx:afterSwap', function(event) {
    // Check if response contains script tag (from delete action)
