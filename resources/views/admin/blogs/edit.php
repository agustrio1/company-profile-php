...
<div class="max-w-4xl">
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-body mb-2">
            ...
            <span class="text-heading">Edit Blog</span>
        </div>
        <h1 class="text-2xl font-bold text-heading">Edit Blog Post</h1>
        <p class="text-sm text-body mt-1">Edit blog post yang sudah ada</p>
    </div>

    <div class="bg-neutral-primary-soft border border-default rounded-base p-6">
        <form action="<?= url('admin/blogs/' . $blog->id) ?>" method="POST" enctype="multipart/form-data" x-data="blogForm()">
            <?= csrf_field() ?>
            <?= method_field('PUT') ?>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <?php if (!empty($blog->thumbnail)): ?>
                        ...
                        ?>
                        <div class="mb-3">
                            <p class="text-xs text-body mb-2">Gambar saat ini:</p>
                            <img src="<?= asset($thumbnailPath) ?>" alt="Current thumbnail" class="w-48 h-32 object-cover rounded-base border border-default">
                        </div>
                    <?php endif; ?>
                    
                    <!-- New Thumbnail Preview -->
                    <div x-show="thumbnailPreview" class="mb-3 relative">
                        <p class="text-xs text-body mb-2">Preview gambar baru:</p>
                        <img :src="thumbnailPreview" class="w-full h-48 object-cover rounded-base border border-default">
                        <button 
                            type="button" 
                            @click="removeThumbnail()"
                            class="absolute top-8 right-2 p-1.5 bg-danger text-white rounded-full hover:bg-danger-strong shadow"
                        >
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                    <p class="text-xs text-body mt-2">Upload gambar baru untuk mengganti. Format: JPG, PNG, WebP. Maksimal 5MB</p>
                ...
                <div>
                    ...
                    <?php 
                    // Set content for wysiwyg editor
                    $editorContent = old('content', $blog->content ?? '');
                    include view_path('components/wysiwyg-editor.php'); 
                    ?>
                ...
                <div class="border-t border-default pt-5">
                    ...
                    <div class="mb-4">
                        ...
                        <input 
                            ...
                            id="seo_title" 
                            name="seo[title]" 
                            value="<?= old('seo.title', $blog->seo->title ?? '') ?>"
                            class="bg-neutral-primary border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
                    ...
                    <div class="mb-4">
                        ...
                        ><?= old('seo.description', $blog->seo->description ?? '') ?></textarea>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="seo_keywords" 
                            name="seo[keywords]" 
                            value="<?= old('seo.keywords', $blog->seo->keywords ?? '') ?>"
                            class="bg-neutral-primary border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
...
function blogForm() {
    return {
        title: '<?= old('title', addslashes($blog->title ?? '')) ?>',
        slug: '<?= old('slug', addslashes($blog->slug ?? '')) ?>',
        autoSlug: false, // Default false on edit
...
function categorySelect() {
    const currentCategoryId = '<?= old('category_id', $blog->category_id ?? '') ?>';
    let currentLabel = '-- Pilih Kategori --';
    
    // Find current category label
    <?php foreach ($categories as $cat): ?>
        if ('<?= $cat->id ?>' === currentCategoryId) {
            currentLabel = '<?= htmlspecialchars($cat->name) ?>';
        }
    <?php endforeach; ?>
